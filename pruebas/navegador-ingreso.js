...
(async () => {
  ...
  if (cargado.correo !== 'elena.bastidas@monpica.example') malo('el correo no se cargó: ' + cargado.correo);
  ...
  await p.fill('#a-correo', 'elena.bastidas@monpica.example'); await p.fill('#a-clave', 'mala');
  await p.click('button[type=submit]'); await p.waitForTimeout(220);
  e = await p.evaluate(() => (document.querySelector('.pi-acceso-error')||{}).innerText || '');
  ...
  await p.fill('#a-correo', 'diego.fuenmayor@monpica.example'); await p.fill('#a-clave', 'monpica2025');

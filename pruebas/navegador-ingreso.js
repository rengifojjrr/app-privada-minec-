...
(async () => {
  ...
  if (cargado.correo !== 'elena.ramos@proyectosintegrales.com.ve') malo('el correo no se cargó: ' + cargado.correo);
  ...
  await p.fill('#a-correo', 'nadie@ejemplo.com'); await p.fill('#a-clave', 'demo1234');
  await p.click('button[type=submit]'); await p.waitForTimeout(220);
  ...
  await p.fill('#a-correo', 'elena.ramos@proyectosintegrales.com.ve'); await p.fill('#a-clave', 'mala');
  await p.click('button[type=submit]'); await p.waitForTimeout(220);
  e = await p.evaluate(() => (document.querySelector('.pi-acceso-error')||{}).innerText || '');
  ...
  await p.fill('#a-correo', 'diego.torres@proyectosintegrales.com.ve'); await p.fill('#a-clave', 'demo1234');
